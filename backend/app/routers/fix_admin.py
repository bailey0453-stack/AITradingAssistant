...
<div class="card"><h2>Order tests</h2><p class="muted">Symbol <span class="pill">USDMXN.r</span> Quantity <span class="pill">10,000</span> Side <span class="pill">BUY</span>. Limit tests use a non-marketable price 0.05 below the current FIX bid.</p><div class="grid">
...
async function getJson(url,opts={}){const r=await fetch(url,opts);const text=await r.text();let j={};if(text){try{j=JSON.parse(text)}catch{j={raw:text}}}if(!r.ok){const detail=j.detail||j.error||j.raw||('HTTP '+r.status);throw new Error(detail+' ['+r.status+' '+url+']')}return j}
...
async function quote(){let lastError=null;for(let i=0;i<3;i++){try{const s=await getJson('./fix-status',{cache:'no-store'});const bid=Number(s?.quote?.bid);const loggedOn=!!s?.fix_logged_on;if(loggedOn&&Number.isFinite(bid)&&bid>0)return bid;lastError=new Error('FIX market data is not ready: logged_on='+loggedOn+' bid='+String(s?.quote?.bid??'missing'))}catch(e){lastError=e}if(i<2)await sleep(500*(i+1))}throw new Error('Unable to obtain live FIX bid after 3 attempts. No order was sent. '+(lastError?.message||''))}
function terminal(report){return report&&['2','4','8'].includes(String(report['39']||''))}
async function pollLifecycle(clOrdId){let matched=[];let lastSession=null;for(let i=0;i<30;i++){await sleep(500);const s=await getJson('./trading-status',{headers:hdr()});lastSession=s;matched=(s.execution_reports||[]).filter(r=>String(r['11']||'')===String(clOrdId||''));if(matched.some(terminal))break}return {cl_ord_id:clOrdId,execution_reports:matched,terminal_report:matched.find(terminal)||null,session:lastSession}}
async function run(kind,btn){if(!confirm('Send this demo/conformance FIX order now?'))return;buttons.forEach(b=>b.disabled=true);$('result').textContent='Preparing '+kind+' test...';$('details').textContent='No order has been sent yet.';try{let body={symbol:'USDMXN.r',side:'1',quantity:10000,ord_type:'1',time_in_force:'3'};if(kind!=='market'){$('result').textContent='Fetching live FIX bid for '+kind+' test...';const bid=await quote();body.ord_type='2';body.time_in_force=kind==='fok'?'4':'3';body.price=Number((bid-0.05).toFixed(5));$('details').textContent=JSON.stringify({quote_bid:bid,planned_limit_price:body.price,request:body,order_sent:false},null,2)}$('result').textContent='Sending '+kind+' test...';const sent=await getJson('./trading-conformance/order',{method:'POST',headers:hdr(),body:JSON.stringify(body)});const clOrdId=sent?.order?.cl_ord_id;if(!clOrdId)throw new Error('Order API returned success without a ClOrdID');$('details').textContent=JSON.stringify({request:body,sent:sent,waiting_for_execution_reports:true},null,2);const outcome=await pollLifecycle(clOrdId);const terminalReport=outcome.terminal_report;const accepted=outcome.execution_reports.some(r=>String(r['39']||'')==='0');$('result').innerHTML=terminalReport?'<span class="ok">Lifecycle captured through terminal status.</span>':accepted?'<span class="ok">Order accepted; no terminal report captured within 15 seconds.</span>':'<span class="bad">No execution report captured for this ClOrdID.</span>';$('details').textContent=JSON.stringify({request:body,sent:sent,outcome:outcome},null,2)}catch(e){$('result').innerHTML='<span class="bad">Test failed: '+e.message+'</span>';$('details').textContent=JSON.stringify({error:e.message,note:'If failure occurred before the order POST, no order was sent.'},null,2)}finally{await check()}}
