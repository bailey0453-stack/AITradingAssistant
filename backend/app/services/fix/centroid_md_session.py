...
from app.services.fix.codec import field_map, parse_fields, split_messages
...
from app.services.fix.parser import parse_market_data_incremental, parse_market_data_snapshot
...
class CentroidMarketDataSession:
    """FIX 4.4 market-data session — read-only; no order routing."""
    ...
    def configured(self) -> bool:
        return bool(self.settings.centroid_md_host and self.settings.centroid_md_port and self.settings.centroid_md_sender_comp_id and self.settings.centroid_md_target_comp_id)

    def _scrub(self, text: str) -> str:
        return scrub(text, getattr(self.settings, "centroid_md_password", None), getattr(self.settings, "centroid_md_username", None))
    ...
    def start_background(self) -> None:
        ...
        if self._thread and self._thread.is_alive():
            return
        self._stop.clear()
        self._thread = threading.Thread(target=self._run_loop, name="centroid-fix-md", daemon=True)
        self._thread.start()
    ...
    def stop(self) -> None:
        ...
        if self._sock:
            try:
                ...
                pass
        self.store.set_health(status="disconnected", tcp_connected=False, fix_logged_on=False, md_subscription_status="none")

    def request_security_list(self) -> dict:
        """Ask Centroid for every security visible to the logged-on FIX session."""
        if not self._sock or not self.store.health().fix_logged_on:
            raise ConnectionError("FIX session is not logged on")
        req_id = f"SEC-{int(time.time() * 1000)}"
        msg = build_security_list_request(
        ...
        self.store.record_security_request(req_id)
        self._send(msg)
        return self.store.security_discovery()
    ...
    def _connect_and_run(self) -> None:
        ...
        self.store.set_health(status="connecting", host=host, port=port, sender_comp_id=self.settings.centroid_md_sender_comp_id, target_comp_id=self.settings.centroid_md_target_comp_id, ssl_enabled=bool(self.settings.centroid_md_ssl), tcp_connected=False, fix_logged_on=False, md_subscription_status="none")
        ...
        md_msg = build_market_data_request(seq_num=self._next_out_seq(), sender_comp_id=self.settings.centroid_md_sender_comp_id or "", target_comp_id=self.settings.centroid_md_target_comp_id or "", symbol=symbol, md_req_id=self._md_req_id, subscription_type=sub_type, market_depth=depth, include_md_update_type=include_265)
        self.store.record_md_request(md_req_id=self._md_req_id, symbol=symbol, subscription_request_type=sub_type, market_depth=depth, md_update_type="1" if include_265 else None, entry_types=["0", "1"])
        self._send(md_msg)
        self.store.set_health(status="connected", outbound_seq=self._out_seq, inbound_seq=self._in_seq)
    ...
    def _handle_message(self, raw: str) -> None:
        ...
        if msg_type == "0":
            ...
            self.store.set_health(status="connected", fix_logged_on=True, last_logon_at=datetime.now(timezone.utc))
        elif msg_type == "W":
            ...
            symbols = [value for tag, value in parse_fields(raw) if tag == "55"]
            self.store.record_security_list(security_req_id=fmap.get("320") or fmap.get("322"), request_result=fmap.get("560"), symbols=symbols)
        elif msg_type == "Y":
            text = fmap.get("58") or fmap.get("372") or "market data request rejected"
            cleaned = self._scrub(text)
            self.store.set_health(md_subscription_status="rejected", last_error=cleaned, warnings=[cleaned])
        elif msg_type == "j":
            text = fmap.get("58") or "business message reject"
            cleaned = self._scrub(text)
            if self.store.security_discovery().get("status") == "pending":
                self.store.record_security_discovery_error(cleaned)
            else:
                self.store.set_health(md_subscription_status="rejected", last_error=cleaned, warnings=[cleaned])
        elif msg_type == "3":
            ...
            raise ConnectionError(self._scrub(fmap.get("58", "logout")))
    ...
    def _send_logon(self) -> None:
        self._send(build_logon(seq_num=self._next_out_seq(), sender_comp_id=self.settings.centroid_md_sender_comp_id or "", target_comp_id=self.settings.centroid_md_target_comp_id or "", username=self.settings.centroid_md_username, password=self.settings.centroid_md_password, heart_bt_int=_HEARTBEAT_INTERVAL, reset_seq_num=bool(self.settings.centroid_md_reset_on_logon)))

    def _send_heartbeat(self, test_req_id: str | None = None) -> None:
        self._send(build_heartbeat(seq_num=self._next_out_seq(), sender_comp_id=self.settings.centroid_md_sender_comp_id or "", target_comp_id=self.settings.centroid_md_target_comp_id or "", test_req_id=test_req_id))
        self.store.set_health(last_heartbeat_at=datetime.now(timezone.utc), outbound_seq=self._out_seq)

    def _send_logout(self) -> None:
        if self._sock:
            self._send(build_logout(seq_num=self._next_out_seq(), sender_comp_id=self.settings.centroid_md_sender_comp_id or "", target_comp_id=self.settings.centroid_md_target_comp_id or "", text="Client shutdown"))
