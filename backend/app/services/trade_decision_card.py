...
def _direction_labels(direction: Optional[str]) -> dict:
    if direction == "SELL_USD":
        return {"bias": "SELL USD / BUY MXN", "prediction": "USD/MXN LOWER", "has_directional_forecast": True}
    if direction == "BUY_USD":
        return {"bias": "BUY USD / SELL MXN", "prediction": "USD/MXN HIGHER", "has_directional_forecast": True}
    return {"bias": "NEUTRAL", "prediction": "RANGE-BOUND", "has_directional_forecast": False}
...
def _blocking_event(payload):
    ...
    if malformed:
        ev = malformed[0]
        return {"name": _event_name(ev), "release_time": ev.get("release_time") or ev.get("time"), "hours_away": None, "importance": "high"}
    return None
...
def _format_event_wait(event):
    ...
    return f"{name} in {h}h" if m == 0 else f"{name} in {h}h {m}m"
...
def build_trade_decision_card(payload: dict) -> dict:
    ...
    if not forecast_ok and rate_close is not None:
        forecast_ok = _forecast_agrees(direction, spot, rate_close)
    ...
    if actionable and not ev_ok:
        wait_reasons.append("Negative expected value" if ev is not None and ev <= 0 else "Expected value unavailable or not positive")
    ...
    if is_invalid and actionable:
        ...
        visual = "red"
        why = f"Spot {spot:g} crossed the invalidation level ({invalidation_label} {invalidation_level:g})." if spot is not None and invalidation_level is not None else "Recommendation is invalidated."
        ...
        action = "TRADE"
        visual = "green"
        why = f"Grade {grade}, confidence {confidence:.0f}, positive expected value, and historical support align with the forecast."
    else:
        action = "WAIT"
        visual = "yellow"
